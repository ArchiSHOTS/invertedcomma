...
export async function runMigrations() {
  // Base tables — bootstraps a brand-new (empty) database; no-ops on existing ones.
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS runtime_quotes (
    id           TEXT PRIMARY KEY,
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS insights_cache (
    quote_id   TEXT PRIMARY KEY,
    data       JSONB NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
  )`);
  await pool.query(`CREATE TABLE IF NOT EXISTS quote_likes (
